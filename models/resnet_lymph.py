...
LOGS = 'lightning_logs'
CHECKPOINTS = 'lightning_logs/checkpoints2'
...
class PCAM_Model(pl.LightningModule):
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(self.parameters(), self.lr, weight_decay=1e-4)
...
if __name__ == '__main__':
    ...
    train_transform = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.5323, 0.4214, 0.6122], std=[0.2068, 0.2196, 0.1762])
    ])
    ...
    train_dataset = HE_Dataset('data/TMA/train', 'labels.json', train_transform)
    train_loader = data.DataLoader(train_dataset, batch_size, pin_memory=True, num_workers=10, shuffle=True)

    valid_dataset = HE_Dataset('data/TMA/valid', 'labels.json', valid_transform)
    valid_loader = data.DataLoader(valid_dataset, batch_size, pin_memory=True, num_workers=5)

    test_dataset = HE_Dataset('data/TMA/test', 'labels.json', test_transform)
    ...
    if train:
        ...
        pcam = PCAM_Model.load_from_checkpoint(checkpoint_path=os.path.join(CHECKPOINTS, checkpoints[0]))
    ...
    for checkpoint in os.listdir(CHECKPOINTS):
        ...
        report = classification_report(all_labels, all_preds, target_names=["Negative", "Positive"], digits=3)
        print(report)