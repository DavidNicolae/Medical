...
class HE_Dataset(data.Dataset):
    ...
    def __init__(self, stage='train') -> None:
        super().__init__()
        if stage == 'train':
            self.transformer = T.Compose([T.Resize(224), T.RandomHorizontalFlip()])
        else:
            self.transformer = T.Compose([T.Resize(224)])
        self.load(stage)
    ...
    def __getitem__(self, index):
        x = torch.from_numpy(self.images[self.x_key][index] / 255).float().permute(2, 0, 1)
        x = self.transformer(x)
        y = torch.from_numpy(self.labels[self.y_key][index]).flatten()
        return x, y
